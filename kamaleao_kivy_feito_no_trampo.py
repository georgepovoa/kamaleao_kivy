...
class KamaleãoApp(App):
    ...
    def build(self):
                    ...
                    for i in formula_formatada:
                        ...
                        try:
                            ...
                            try:
                                ...
                                cursor_kamaleao.execute("SELECT SUM(quantidade_saida) FROM relatorio_saida_materia_prima WHERE nome_mp = ?",nome_pra_sales_rate)
                                ...
                                conn_kamaleao.execute("UPDATE relatorio_saida_materia_prima SET sales_rate = ? WHERE nome_mp = ? AND data = ?",update_sales_rate)
        ...
        def relatorios_view(instance):
            ...
            cursor_kamaleao.execute("SELECT nome_mp, sales_rate FROM relatorio_saida_materia_prima ORDER BY nome_mp ASC")
            ...
            for i in range(len(nome_estoque_atual)):
                ...
                try:
                    days_sales_layout.add_widget(Label(text = str(round(nome_estoque_atual[i][1]/metricas_sales_rate[i][1],2))))
                    try:
